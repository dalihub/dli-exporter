#ifndef LIBDLI_MATERIAL_DEFINITION_H
#define LIBDLI_MATERIAL_DEFINITION_H
/*
...
struct LIBDLI_API SamplerFlags
{
  ...
  static constexpr Type Encode(Dali::FilterMode::Type minFilter, Dali::FilterMode::Type magFilter,
    Dali::WrapMode::Type wrapS, Dali::WrapMode::Type wrapT);

  ///@brief Decodes the minification filter patter of @a flags into the corresponding Dali::FilterMode.
  static constexpr Dali::FilterMode::Type  GetMinFilter(Type flags);

  ///@brief Decodes the magnification filter patter of @a flags into the corresponding Dali::FilterMode.
  static constexpr Dali::FilterMode::Type  GetMagFilter(Type flags);

  ///@brief Decodes the horizontal wrap pattern of @a flags into the corresponding Dali::WrapMode.
  static constexpr Dali::WrapMode::Type GetWrapS(Type flags);

  ///@brief Decodes the vertical wrap pattern of @a flags into the corresponding Dali::WrapMode.
  static constexpr Dali::WrapMode::Type GetWrapT(Type flags);

  ///@brief Creates a Dali::Sampler with the settings encoded in @a flags.
  static Dali::Sampler MakeSampler(Type flags);
};
...
struct LIBDLI_API MaterialDefinition
{
  enum Flags : uint32_t
  {
    ...
    ANIMATED_IMAGES = NthBit(21),
    GLTF_CHANNELS = NthBit(22),  // https://github.com/KhronosGroup/glTF/tree/master/specification/2.0#pbrmetallicroughnessmetallicroughnesstexture
  ...
  using Vector = std::vector<std::pair<MaterialDefinition, Dali::TextureSet>>;

  struct RawData
  {
    struct TextureData
    {
      Dali::PixelData mPixels;
      SamplerFlags::Type mSamplerFlags;
    };
  ...
  Dali::TextureSet Load(const EnvironmentDefinition::Vector& environments, RawData&& raw) const;
...
#endif //LIBDLI_MATERIAL_DEFINITION_H
