...
const std::map<Dali::Property::Type, Constraint(*)(Actor&, Property::Index)> sConstraintFactory = {
  {
    Dali::Property::Type::BOOLEAN,
...
void VisitInternal(Index iNode, const Customization::Choices& choices, Visitor& v, SceneDefinition& sd)
{
  ...
  if (node.mCustomization)
  {
    ...
    {
      auto iFind = choices.find(node.mCustomization->mTag);
      Index choice = std::min(iFind != choices.end() ? iFind->second : 0,
        static_cast<Index>(node.mChildren.size() - 1));
      sd.Visit(node.mChildren[choice], choices, v);
    }
  }
  else
  {
...
uint32_t dli::SceneDefinition::AddRootNode(Index iNode)
{
...
const std::vector<Index>& dli::SceneDefinition::GetRoots() const
{
  return mRootNodeIds;
}

void dli::SceneDefinition::RemoveRootNode(Index iRoot)
{
  ...
  struct : NodeDefinition::IConstVisitor
  {
    ...
    void Start(const NodeDefinition& n)
    {
      if (n.mCustomization)
      {
        ...
        auto iFind = choices->find(tag);
        if (iFind == choices->end())
        {
          (*missingChoices)[tag] = 0;
        }

        auto& customization = (*options)[tag];
...
bool SceneDefinition::ConfigureBlendshapeShaders(const ResourceBundle& resources,
  Dali::Actor rootActor, std::vector<BlendshapeShaderConfigurationRequest>&& requests,
