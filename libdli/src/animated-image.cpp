 ...
 */
 ...
 // CLASS HEADER
#include "libdli/animated-image.h"
#include "dali/integration-api/debug.h"

namespace dli
{

AnimatedImage::AnimatedImage(Dali::Actor actor, Index materialIndex, int frameRate, int frameStart, int frameEnd, int loopCount, UpdateAnimatedMaterialFn callback)
:  mActor(actor),
  ...
  mCallback(callback)
{
  mTimer = Dali::Timer::New(1000 / mFrameRate);
  mTimer.TickSignal().Connect(this, &AnimatedImage::OnTimer);
}

const Dali::Actor AnimatedImage::GetActor() const
{
  return mActor;
}
