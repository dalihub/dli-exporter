...
#include "libdli/resource-bundle.h"
#include "libdli/gltf2-loader.h"
#include "libdli/gltf2-asset.h"
...
const auto ACCESSOR_READER = std::move(js::Reader<gt::Accessor>()
  ...
  .Register(*new js::Property<gt::Accessor, StringView>("name", js::Read::StringView, &gt::Accessor::mName))
...
const auto IMAGE_READER = std::move(js::Reader<gt::Image>()
  .Register(*new js::Property<gt::Image, StringView>("name", js::Read::StringView, &gt::Material::mName))
...
const auto MATERIAL_READER = std::move(js::Reader<gt::Material>()
  .Register(*new js::Property<gt::Material, StringView>("name", js::Read::StringView, &gt::Material::mName))
...
const auto MESH_READER = std::move(js::Reader<gt::Mesh>()
  .Register(*new js::Property<gt::Mesh, StringView>("name", js::Read::StringView, &gt::Mesh::mName))
...
const auto SKIN_READER = std::move(js::Reader<gt::Skin>()
  .Register(*new js::Property<gt::Skin, StringView>("name", js::Read::StringView, &gt::Skin::mName))
...
const auto CAMERA_READER = std::move(js::Reader<gt::Camera>()
  .Register(*new js::Property<gt::Camera, StringView>("name", js::Read::StringView, &gt::Camera::mName))
...
const auto NODE_READER = std::move(js::Reader<gt::Node>()
  .Register(*new js::Property<gt::Node, StringView>("name", js::Read::StringView, &gt::Node::mName))
...
const auto ANIMATION_READER = std::move(js::Reader<gt::Animation>()
  .Register(*new js::Property<gt::Animation, StringView>("name", js::Read::StringView, &gt::Animation::mName))
...
const auto SCENE_READER = std::move(js::Reader<gt::Scene>()
  .Register(*new js::Property<gt::Scene, StringView>("name", js::Read::StringView, &gt::Scene::mName))
...
{
  return TextureDefinition{ mm.mTexture->mSource->mUri.ToString(), ConvertSampler(mm.mTexture->mSampler) };
}
...
void ConvertNode(gt::Node const& node, const Index gltfIdx, Index parentIdx, ConversionContext& cctx)
{
  ...
  auto weakNode = scene.AddNode([&]() {
    ...
    nodeDef->mName = node.mName.ToString();
...
float LoadDataFromAccessors(const std::string& path, const gltf2::Accessor& input, const gltf2::Accessor& output, Vector<float>& inputDataBuffer, Vector<T>& outputDataBuffer)
{
  ...
  LoadDataFromAccessor<float>(path + input.mBufferView->mBuffer->mUri.ToString(), inputDataBuffer,
    input.mBufferView->mByteOffset + input.mByteOffset, inputDataBufferSize);
  LoadDataFromAccessor<T>(path + output.mBufferView->mBuffer->mUri.ToString(), outputDataBuffer,
...
float LoadBlendShapeKeyFrames(const std::string& path, const gt::Animation::Channel& channel, const std::string& nodeName, uint32_t& propertyIndex, std::vector<dli::AnimatedProperty>& properties)
{
  const gltf2::Accessor& input = *channel.mSampler->mInput;
  const gltf2::Accessor& output = *channel.mSampler->mOutput;
...
void ConvertAnimations(const gt::Document& doc, ConversionContext& cctx)
{
  ...
  for (const auto& animation : doc.mAnimations)
  {
    ...
    {
      std::string nodeName;
      if (channel.mTarget.mNode->mName)
      {
        nodeName = channel.mTarget.mNode->mName.ToString();
      }
      else
      {
...
void ProcessSkins(const gt::Document& doc, ConversionContext& cctx)
{
  ...
  struct InverseBindMatrixAccessor : public IInverseBindMatrixProvider
  {
    ...
    InverseBindMatrixAccessor(const gt::Accessor& accessor, const std::string& path)
      : mStream(path + accessor.mBufferView->mBuffer->mUri.ToString(), std::ios::binary),
...
void LoadGltfScene(const std::string& url, ShaderDefinitionFactory& shaderFactory, LoadResult& params)
{
  ...
  auto path = url.substr(0, url.rfind('/'));
