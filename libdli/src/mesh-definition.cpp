...
// INTERNAL INCLUDES
#include "libdli/mesh-definition.h"
...
void GenerateNormals(MeshDefinition::RawData& raw)
{
  ...
  attribs.push_back({ "aNormal", Dali::Property::VECTOR3, attribs[0].mNumElements, std::move(buffer) });
}
...
void GenerateTangentsWithUvs(MeshDefinition::RawData& raw)
{
  ...
  attribs.push_back({ "aTangent", Dali::Property::VECTOR3, attribs[0].mNumElements, std::move(buffer) });
}
  ...
  MeshDefinition::LoadRaw(const std::string& modelsPath) const
{
  ...
  const auto isTriangles = mPrimitiveType == Dali::Geometry::TRIANGLES;
