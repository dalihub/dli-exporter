...
#include "libdli/skinning-details.h"
...
#include "libdli/json-util.h"
#include "libdli/ktx-loader.h"
#include "libdli/text-cache-item.h"
#include "libdli/utils.h"
#include "libdli/text-details.h"
...
const rs::Type LEGACY_RENDER_MODES[] = {
  rs::DEFAULT,
  /*BLENDING*/
  rs::ALPHA_BLEND,
  /*BLENDING_DEPTHTEST_SHADOW*/
  rs::DEPTH_TEST | rs::ALPHA_BLEND |
    rs::FromBlendFactors(BlendFactor::ONE_MINUS_DST_ALPHA, BlendFactor::OMIT, BlendFactor::OMIT, BlendFactor::OMIT),
  /*STENCIL_DEPTHTEST*/
  rs::DEPTH_TEST | (BufferMode::STENCIL << rs::BUFFER_MODE_SHIFT),
  /*COLOR_STENCIL_DEPTHTEST*/
  rs::DEPTH_TEST | rs::ALPHA_BLEND | (BufferMode::COLOR_STENCIL << rs::BUFFER_MODE_SHIFT),
  /*BLENDING_DEPTHTEST*/
  rs::DEPTH_TEST | rs::ALPHA_BLEND,
  /*BLENDING_DEPTHTEST_CULLBACK*/
  rs::DEPTH_TEST | rs::ALPHA_BLEND | rs::CULL_BACK,
};
...
bool ReadFontSize(const TreeNode* node, float& fontSize, FontSizeUnits& units)
{
  ...
  case TreeNode::STRING:
  {
    ...
    const bool fromPixelsToPoints = ((2u < size) && dli::CaseInsensitiveStringCompare((fontSizeStr.c_str() + size - 2u), PIXEL_UNITS));
...
void DliLoader::ParseScene(LoadParams& params)
{
  ...
  {
    c.second(docRoot->GetChild(c.first), mOnError);
  }
  ...
  auto tnScenes = docRoot->GetChild("scenes");
  auto tnNodes = docRoot->GetChild("nodes");
...
void DliLoader::ParseSceneInternal(Index iScene, const Dali::Toolkit::TreeNode* tnScenes,
...
void DliLoader::ParseSkeletons(const TreeNode* skeletons, SceneDefinition& scene, ResourceBundle& resources)
{
  if (skeletons)
  {
    ...
    for (auto i0 = iStart, i1 = skeletons->CEnd(); i0 != i1; ++i0)
    {
      ...
      if (ReadString(node.GetChild(NODE), skeletonRootName))
      {
        ...
        {
          ExceptionFlinger(ASSERT_LOCATION) << FormatString("Skeleton %d: node '%' not defined.", resources.mSkeletons.size(), skeletonRootName.c_str());
        }
        ...
        visitFn = [&](Index id) {
          ...
          jointCount += mInverseBindMatrices.find(id) != mInverseBindMatrices.end();
        ...
        visitFn = [&](Index id) {
          auto iFind = mInverseBindMatrices.find(id);
          if (iFind != mInverseBindMatrices.end() && skeleton.mJoints.size() < Skinning::MAX_JOINTS)
          {
...
void DliLoader::ParseEnvironments(const TreeNode* environments, ResourceBundle& resources)
{
...
void DliLoader::ParseShaders(const TreeNode* shaders, ResourceBundle& resources)
{
  uint32_t iShader = 0;
  for (auto i0 = shaders->CBegin(), i1 = shaders->CEnd(); i0 != i1; ++i0, ++iShader)
  {
      ...
      ReadString(node.GetChild("fragment"), shaderDef.mFragmentShaderPath))
    {
      ...
      for (TreeNode::ConstIterator j0 = node.CBegin(), j1 = node.CEnd(); j0 != j1; ++j0)
      {
        ...
        ShaderDefinition::Uniform uniform;
        ...
        else if (key.compare("renderMode") == 0)
        {
          int renderMode = 0;
          if (!ReadInt(&keyValue.second, renderMode) || renderMode < 0 ||
            static_cast<size_t>(renderMode) >= std::extent<decltype(LEGACY_RENDER_MODES)>::value)
          {
            mOnError(FormatString("shader %d: invalid renderMode value; default used.",
              iShader, keyValue.second.GetString()));
          }
          else
          {
            shaderDef.mRendererState = LEGACY_RENDER_MODES[static_cast<RendererState::Type>(renderMode)];
          }
          continue;
        }
        ...
        if (Property::NONE != uniform.mData.GetType())
        {
          shaderDef.mUniforms[std::move(key)].reset(uniform.Clone());
        }
      }
...
void DliLoader::ParseMeshes(const TreeNode* meshes, ResourceBundle& resources)
{
...
void DliLoader::ParseMaterials(const TreeNode* materials, ConvertColorCode convertColorCode,
...
void DliLoader::ParseNodes(const TreeNode* const nodes, Index index, LoadParams& params)
{
  ...
  struct IndexMapper : IIndexMapper
  {
    ...
    virtual unsigned int Resolve(Index iDli) override
    {
      auto iFind = std::lower_bound(mIndices.begin(), mIndices.end(), iDli,
        [](const Entry& idx, dli::Index iDli) {
...
void DliLoader::ParseNodesInternal(const TreeNode* const nodes, Index index, LightingMode::Type lightingMode,
    ...
    for (auto& idRes : resourceIds)
    {
      ...
      else if (idRes.source->GetType() != TreeNode::INTEGER)
      {
        ExceptionFlinger(ASSERT_LOCATION) << "node " << index << ": Invalid " <<
          GetResourceTypeName(idRes.type) << " index value '" << idRes.source->GetString() << "'";
      }
      else
      {
        ...
        for (auto j0 = children->CBegin(), j1 = children->CEnd(); j0 != j1; ++j0, ++iChild)
        {
          ...
          else
          {
            ExceptionFlinger(ASSERT_LOCATION) << "node " << index << ", child " << iChild <<
              ": Index is invalid value '" << child.GetString() << "'.";
          }
        }
...
void DliLoader::ParseAnimations(const TreeNode* tnAnimations, LoadParams& params)
{
    ...
    iAnim != iAnimEnd; ++iAnim)
  {
    ...
    {
      proc(*iFind, tnAnim, mOnError);
    }
  }
}

void DliLoader::ParseAnimationGroups(const Dali::Toolkit::TreeNode* tnAnimationGroups, LoadParams& params)
{
...
void DliLoader::GetCameraParameters(std::vector<CameraParameters>& cameras) const
{
...
void DliLoader::GetLightParameters(std::vector<LightParameters>& lights) const
{
