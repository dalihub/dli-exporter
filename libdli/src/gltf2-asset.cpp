...
const std::map<dli::StringView, AccessorType::Type> ACCESSOR_TYPES{
...
const std::map<dli::StringView, Animation::Sampler::Interpolation::Type> ANIMATION_SAMPLER_INTERPOLATION{
...
const std::map<dli::StringView, Animation::Channel::Target::Type> ANIMATION_CHANNEL_TARGET_PATH_TYPES{
...
{
  auto iFind = ACCESSOR_TYPES.find(dli::StringView(s, len));
...
{
  auto iFind = ALPHA_MODE_TYPES.find(dli::StringView(s, len));
...
{
  auto iFind = ATTRIBUTE_TYPES.find(dli::StringView(s, len));
...
{
  auto iFind = ANIMATION_SAMPLER_INTERPOLATION.find(dli::StringView(s, len));
...
Animation::Channel::Target::Type Animation::Channel::Target::FromString(const char* s, size_t len)
{
  ...
  auto iFind = ANIMATION_CHANNEL_TARGET_PATH_TYPES.find(dli::StringView(target.c_str(), len));
...
void Node::SetMatrix(const Dali::Matrix& m)
{
...
{
	return Dali::Quaternion(ReadDaliVector<Dali::Vector4>(j));
}
